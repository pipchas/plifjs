...
function drawGraph(content, stateStr, next_state){
  ...
  let graphCont = result.querySelector("#graphMainContainer");
  console.log(graphCont, 'GRGGRGRGRGR')
  ...
  //vars некая мапа которая  в себе содержит почти все откуда куда и права
  //необходимо посмотреть про некст стр
  ...
  console.log(data, 'FAFAFAF')
  content.innerHTML = "";
  //тут происходит отрисовка графа
  content.appendChild(graphCont);
  let temp = content.querySelector("#graphMainContainer");
  GraphData = data;
  //vis библа для визуализации канвас
